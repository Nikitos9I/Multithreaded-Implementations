...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private val DEFAULT_ACCOUNT: Account = Account(0);
    ...
    private abstract inner class Op {
        ...
        val atomicFrom = AtomicReference<Account>(DEFAULT_ACCOUNT)
        val atomicTo = AtomicReference<Account>(DEFAULT_ACCOUNT)
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
            ...
            if (fromIndex < toIndex) {
                ...
                atomicFrom.compareAndSet(DEFAULT_ACCOUNT, from)
                atomicTo.compareAndSet(DEFAULT_ACCOUNT, to)

                if (amount > atomicFrom.get().amount) {
                ...
                if (atomicTo.get().amount + amount > MAX_AMOUNT) {
                ...
                accounts[fromIndex].compareAndSet(atomicFrom.get(), AcquiredAccount(atomicFrom.get().amount - amount, this))
                accounts[toIndex].compareAndSet(atomicTo.get(), AcquiredAccount(atomicTo.get().amount + amount, this))
                completed = true
